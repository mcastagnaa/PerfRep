...
IF  EXISTS (
	...
	WHERE 	id = OBJECT_ID(N'dbo.spS_GetPerfDataRaw_Finance') AND 
		OBJECTPROPERTY(id,N'IsProcedure') = 1
	)
DROP PROCEDURE dbo.spS_GetPerfDataRaw_Finance
GO

CREATE PROCEDURE dbo.spS_GetPerfDataRaw_Finance
	@RefDate datetime
AS
...
SELECT	FS.Company + ' ' + FS.FundName AS FundName
	...
	, FS.NP1y AS FundPerformance1y
	, FS.Ben1y AS BenchmarkPerformance1y
	, FS.PGmed_r1y AS PeerGroupMedianRet1y
	, FS.PGavg_r1y AS PeerGroupAverageRet1y
...
-- FOR OCT REPORT
	AND FS.ShortCode IN ('ASFO', 'EEFO', 'GEFO', 'GSBO', 'JSFO', 'NAEO', 'UKSEO', 'SMFO')
...
GRANT EXECUTE ON dbo.spS_GetPerfDataRaw_Finance TO [OMAM\Compliance]
