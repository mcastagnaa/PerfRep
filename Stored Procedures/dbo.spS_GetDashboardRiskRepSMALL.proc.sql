...
IF  EXISTS (
	...
	WHERE 	id = OBJECT_ID(N'dbo.spS_GetDashboardRiskRepSMALL') AND 
		OBJECTPROPERTY(id,N'IsProcedure') = 1
	)
DROP PROCEDURE dbo.spS_GetDashboardRiskRepSMALL
GO

CREATE PROCEDURE dbo.spS_GetDashboardRiskRepSMALL
	@RefDate datetime
	, @Lookback integer
AS
...
--		, FS.PortVaRAvg
--		, FS.BenVaRAvg
--		, FS.PortVaRAvg/FS.BenVaRAvg AS VaRRatio
		, Prod.RiskLimitName AS LimitType
		, Prod.RiskLimitValue AS Limit
		, (CASE Prod.RiskLimitName 
				WHEN 'Absolute' THEN
					((CASE FS.ConfidenceInt WHEN 0.95 THEN 1.414319 ELSE 1 END) * 
						FS.LastVaR) 
				WHEN 'Relative' THEN
		...
		, (CASE WHEN Prod.RiskLimitName = 'Exposure' THEN NULL 
				ELSE FS.ExpectedVaREvents END) AS ExpectedVaREvents
		, (CASE WHEN Prod.RiskLimitName = 'Exposure' THEN NULL 
				ELSE FS.VaREventsNeg END) AS VaREventsNeg
		, FS.LastGrossExp
		, FS.LastNetExp
--		, FS.GrossExpAvg
--		, FS.NetExpAvg
...
GRANT EXECUTE ON dbo.spS_GetDashboardRiskRepSMALL TO [OMAM\Compliance]