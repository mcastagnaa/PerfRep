...
IF  EXISTS (
	...
	WHERE 	id = OBJECT_ID(N'dbo.spS_GetScharts3yData') AND 
		OBJECTPROPERTY(id,N'IsProcedure') = 1
	)
DROP PROCEDURE dbo.spS_GetScharts3yData
GO

CREATE PROCEDURE dbo.spS_GetScharts3yData
	@RefDate datetime
...
FROM
	...
	SELECT	MStarCategory
		...
		, ROW_NUMBER() OVER (
			...
			ORDER BY [3yCum] ASC, FundName ASC) AS RowAsc
		, ROW_NUMBER() OVER (
			PARTITION BY MStarCategory
			ORDER BY [3yCum] DESC, FundName DESC) AS RowDesc
	FROM [OMAM\Matteoc].tbl_SCHARTSrawdata
	WHERE [3yCum] IS NOT NULL 
		AND RefDate = @RefDate
	) X
...
SELECT	Dset.IM
	...
	, DSet.MStarCategory
	, SMed.MedianReturn
	, (CASE WHEN Dset.[3yCum] >= SMed.MedianReturn THEN 1
...
----------------------------------------------------------------------------------
GRANT EXECUTE ON dbo.spS_GetScharts3yData TO [OMAM\Compliance]