...
IF  EXISTS (
	...
	WHERE 	id = OBJECT_ID(N'dbo.spS_GetScharts2yData') AND 
		OBJECTPROPERTY(id,N'IsProcedure') = 1
	)
DROP PROCEDURE dbo.spS_GetScharts2yData
GO

CREATE PROCEDURE dbo.spS_GetScharts2yData
	@RefDate datetime
...
FROM
	...
	SELECT	MStarCategory
		...
		, ROW_NUMBER() OVER (
			...
			ORDER BY [2yCum] ASC, FundName ASC) AS RowAsc
		, ROW_NUMBER() OVER (
			PARTITION BY MStarCategory
			ORDER BY [2yCum] DESC, FundName DESC) AS RowDesc
	FROM	tbl_SChartsrawdata
	WHERE [2yCum] IS NOT NULL 
		AND RefDate = @RefDate
	) X
...
SELECT	Dset.IM
	...
	, DSet.MStarCategory
	, SMed.MedianReturn
	, (CASE WHEN Dset.[2yCum] >= SMed.MedianReturn THEN 1
...
----------------------------------------------------------------------------------
GRANT EXECUTE ON dbo.spS_GetScharts2yData TO [OMAM\Compliance]