...
IF  EXISTS (
	...
	WHERE 	id = OBJECT_ID(N'dbo.spS_GetScharts1yData') AND 
		OBJECTPROPERTY(id,N'IsProcedure') = 1
	)
DROP PROCEDURE dbo.spS_GetScharts1yData
GO

CREATE PROCEDURE dbo.spS_GetScharts1yData
	@RefDate datetime
...
FROM
	...
	SELECT	MStarCategory
		...
		, ROW_NUMBER() OVER (
			...
			ORDER BY [1yr] ASC, FundName ASC) AS RowAsc
		, ROW_NUMBER() OVER (
			PARTITION BY MStarCategory
			ORDER BY [1yr] DESC, FundName DESC) AS RowDesc
	FROM	tbl_SChartsRawData
	WHERE [1yr] IS NOT NULL 
		AND RefDate = @RefDate
	) X
...
SELECT	Dset.IM
	...
	, DSet.MStarCategory
	, SMed.MedianReturn
	, (CASE WHEN Dset.[1yr] >= SMed.MedianReturn THEN 1
...
----------------------------------------------------------------------------------
GRANT EXECUTE ON dbo.spS_GetScharts1yData TO [OMAM\Compliance]