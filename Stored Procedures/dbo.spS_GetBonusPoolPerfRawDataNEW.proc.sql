...
IF  EXISTS (
	...
	WHERE 	id = OBJECT_ID(N'dbo.spS_GetBonusPoolPerfRawData') AND 
		OBJECTPROPERTY(id,N'IsProcedure') = 1
	)
DROP PROCEDURE dbo.spS_GetBonusPoolPerfRawData
GO

CREATE PROCEDURE dbo.spS_GetBonusPoolPerfRawData
	@RefDate datetime
--	, @Filter integer
--	, @Company nvarchar(50)
--	, @Desk nvarchar(50)
...
SELECT	FS.*
	, Funds.BonusPool
	, dbo.fn_GetQuartile(CAST(VW.ProdIRRank1y AS float)/VW.PGIRNum1y, 1) AS ProdIR1yQuart
	, dbo.fn_GetQuartile(CAST(VW.ProdIRRank2y AS float)/VW.PGIRNum2y, 1) AS ProdIR2yQuart
	, dbo.fn_GetQuartile(CAST(VW.ProdIRRank3y AS float)/VW.PGIRNum3y, 1) AS ProdIR3yQuart
	, dbo.fn_GetQuartile(CAST(VW.ProdIRRank5y AS float)/VW.PGIRNum5y, 1) AS ProdIR5yQuart
	, dbo.fn_GetQuartile(CAST(VW.ProdIRRankYtD AS float)/VW.PGIRNumYtD, 1) AS ProdIRYtDQuart
	, dbo.fn_GetQuartile(CAST(VW.ProdIRRankEo2y_TD AS float)/VW.PGIRNumEo2y_TD, 1) AS ProdIREo2y_TDQuart
	, dbo.fn_GetQuartile(CAST(VW.ProdIRRankEo3y_TD AS float)/VW.PGIRNumEo3y_TD, 1) AS ProdIREo3y_TDQuart

	, dbo.fn_GetQuartile(CAST(FS.ProdRank1y AS float)/FS.PeersNo1y, 1) AS Prod1yQuart
	, dbo.fn_GetQuartile(CAST(FS.ProdRank2y AS float)/FS.PeersNo2y, 1) AS Prod2yQuart
	, dbo.fn_GetQuartile(CAST(FS.ProdRank3y AS float)/FS.PeersNo3y, 1) AS Prod3yQuart
	, dbo.fn_GetQuartile(CAST(FS.ProdRank5y AS float)/FS.PeersNo5y, 1) AS Prod5yQuart
	, dbo.fn_GetQuartile(CAST(FS.ProdRankYtD AS float)/FS.PeersNoYtD, 1) AS ProdYtDQuart
	, dbo.fn_GetQuartile(CAST(FS.ProdRankEo2y_TD AS float)/FS.PeersNoEo2y_TD, 1) AS ProdEo2y_TDQuart
	, dbo.fn_GetQuartile(CAST(FS.ProdRankEo3y_TD AS float)/FS.PeersNoEo3y_TD, 1) AS ProdEo3y_TDQuart
...
SELECT	Fset.Id AS FundId
	, Fset.FundName 
--	, ProdRankYtD/PeersNoYtD AS PercYtD
--	, ProdRankEo2y_TD/PeersNoEo2y_TD AS PercEo2y_TD
--	, ProdRankEo3y_TD/PeersNoEo3y_TD AS PercEo3y_TD
--	, ProdRank1y/PeersNo1y AS Perc1y
--	, ProdRank2y/PeersNo2y AS Perc2y
...
GRANT EXECUTE ON spS_GetBonusPoolPerfRawData TO [OMAM\Compliance]
